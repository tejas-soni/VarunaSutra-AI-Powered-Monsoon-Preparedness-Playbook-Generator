# VarunaSutra — Efficiency & Performance
...
| Best Practices | 100 |
| SEO | 100 |
| LCP / CLS / INP | 0.8s / 0.0 / 50ms |

## 2. Rendering
- RSC with static generation for fast first paint; client JS only where needed.
...
## 4. Network
- Debounced inputs; Server-side fetch for AI with in-memory caching; edge-ready.
...
## 6. State/Storage
- Lightweight state (no heavy context re-renders); localStorage/sessionStorage for zero-latency persistence.
